...
SELECT count(*) as nb, (sp_get_player_win_number_last_month(1, '2016-02-01'::date)* 100 / (CASE count(*) WHEN 0 THEN 100 ELSE count(*) END)) as result, sp_get_player_win_number_last_month(1, '2016-02-01'::date) as win
FROM matches m
WHERE m.date <= '2016-02-01'::date AND m.date >= ('2016-02-01'::date - interval '1 month') AND (m.player1_id = 1 OR m.player2_id = 1);
...
WHERE m.date >= '2016-03-01'::date;

SELECT * FROM sp_get_player_win_last_month(
    (SELECT id FROM players WHERE name = 'Carlos Berlocq'), '2016-05-23');
