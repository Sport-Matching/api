...
class SpGetMatchesVsWithName extends LuSpDbo {

    /**
    * @param $dam
    * @return \App\Http\DataAccess\SP\SpGetMatchesVsWithName|null
    */
    protected static function damToDbo($dam)
    {
        if (is_null($dam))
            return null;
        $dbo = new SpGetMatchesVsWithName();
    ...
    /**
    * @param $_player1_id text
    * @param $_player2_id text
    * @return \App\Http\DataAccess\SP\SpGetMatchesVsWithName[];
    */
    public static function execute($_player1_id, $_player2_id)
    {
        $values = DB::select('SELECT to_json(data.id) AS id, to_json(data.player1) AS player1, to_json(data.player2) AS player2, to_json(data.date) AS date, to_json(data.sets) AS sets, to_json(data.tournament) AS tournament FROM sp_get_matches_vs_with_name(:_player1_id, :_player2_id) data', array(":_player1_id" => $_player1_id, ":_player2_id" => $_player2_id));
        $dboValues = array();
    ...
    */
    ...
    {
        $values = DB::select('SELECT (SELECT count(*) FROM sp_get_matches_vs_with_name(:_player1_id, :_player2_id)) as count, (SELECT json_agg(q) FROM (SELECT * FROM sp_get_matches_vs_with_name(:_player1_id, :_player2_id) OFFSET (:page::int * :perPage::int) LIMIT :perPage) q) as data',
