...
class SpGetPlayerWinLastMonth extends LuSpDbo {

    /**
    * @param $dam
    * @return \App\Http\DataAccess\SP\SpGetPlayerWinLastMonth|null
    */
    protected static function damToDbo($dam)
    {
        if (is_null($dam))
            return null;
        $dbo = new SpGetPlayerWinLastMonth();
    ...
    * @return \App\Http\DataAccess\SP\SpGetPlayerWinLastMonth;
    */
    public static function execute($in_id, $in_date)
    {
        $values = DB::select('SELECT to_json(data.nb) AS nb, to_json(data.total) AS total, to_json(data.result) AS result FROM sp_get_player_win_last_month(:in_id, :in_date) data', array(":in_id" => $in_id, ":in_date" => $in_date));
